...
@interface MTMCMediaDownloader () <NSURLSessionDataDelegate>
...
@implementation MTMCMediaDownloader
...
- (instancetype)initWithURL:(NSURL *)url {
    ...
    if (self) {
        ...
        configuration.protocolClasses = [@[[MTMCURLProtocol class]] arrayByAddingObjectsFromArray:configuration.protocolClasses];
...
- (NSURLSessionDataTask *)fetchFileInfoTaskWithCompletion:(void(^)(MTMCContentInfo *info, NSError *error))completion {
    ...
    NSURLSessionDataTask *task = [self.session dataTaskWithRequest:request completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        if (!error) {
            MTMCContentInfo *info = [MTMCContentInfo new];
