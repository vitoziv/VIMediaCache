...
#import "MTMCURLProtocol.h"
#import "MTMCMediaCacheWorker.h"
#import "MTMCCacheAction.h"
#import "MTMCCacheSessionManager.h"

@interface MTMCURLProtocol () <NSURLSessionDataDelegate>

@property (nonatomic, strong) NSURLSession *session;
@property (nonatomic, strong) MTMCMediaCacheWorker *cacheWorker;
@property (nonatomic, strong) NSMutableArray<MTMCCacheAction *> *restActions;
...
@implementation MTMCURLProtocol

- (void)commonInit {
    if (!_session) {
        NSURLSessionConfiguration *configuration = [NSURLSessionConfiguration defaultSessionConfiguration];
        NSOperationQueue *queue = [MTMCCacheSessionManager shared].downloadQueue;
        _session = [NSURLSession sessionWithConfiguration:configuration delegate:self delegateQueue:queue];
    }
    
    if (!_cacheWorker) {
        NSString *cacheName = [self.request.URL lastPathComponent];
        _cacheWorker = [MTMCMediaCacheWorker inMemoryCacheWorkerWithCacheName:cacheName];
...
- (void)processActions {
    MTMCCacheAction *action = [self.restActions firstObject];
    if (!action) {
        [self.client URLProtocolDidFinishLoading:self];
        return;
    }
    ...
    if (action.actionType == MTMCCacheAtionTypeLocal) {
